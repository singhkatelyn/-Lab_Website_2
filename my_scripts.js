...
function loadStatsPage()
{
  ...
  var buf = 0;
	...
	{//Outer for loop iterates over each row
		buf = table.rows[row_counter].cells[2].innerHTML;
		op = table.rows[row_counter].cells[3].innerHTML;
				if((buf-op) > 0)
				{
...
{
	let select = document.getElementById("player_selector");
	for(let i = 0; i < players.length; i++)
	{
		let pname = players[i].name;
		select.innerHTML += "<a class=\"dropdown-item\" href=\"#\" onclick=\"switchPlayers("+i+")\">" + pname + "</a>";
	}
}
